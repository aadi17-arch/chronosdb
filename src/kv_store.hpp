...
class KVStore {
private:
    // This is my custom SkipNode that holds the data and its tower of pointers
    struct SkipNode {
        std::string key;
        std::string value;
        std::vector<SkipNode*> tower_of_pointers; // I use this to store pointers for each level
    ...
    const float p = 0.5f; // My coin flip probability for level distribution
    int current_level;
    SkipNode* sentinel; // My dummy start node
    std::string filepath_;
    mutable std::mutex mutex_;
    std::string wal_filepath_;
    mutable std::ofstream wal_out;

    // My random generator for SkipList levels
    std::mt19937 random_number_generator;
    std::uniform_real_distribution<float> random_distribution;

    enum class WalOp : uint8_t { PUT = 0, DEL = 1 };

    // Helper to write my strings to binary files
    ...
    // Helper to read my strings from binary files
    ...
    // I use this to append every change to my WAL log for durability
    ...
    // My recovery logic that replays my WAL after a crash
    ...
    // My coin-flip method to decide how high a new node should be
    ...
    // My staircase search that skips through levels to find keys quickly
...
public:
    ...
    // My range scan method that returns data between two keys
    ...
    // My prefix search using the range scan logic
    ...
    // How I get a value for a specific key
    ...
    // My insertion method that updates the SkipList levels
    void put(const std::string& key, const std::string& value) {
        std::lock_guard<std::mutex> lock(mutex_);
        append_to_wal(WalOp::PUT, key, value);

        // I store every pointer we might need to update here
        ...
        int new_node_height = random_level_generator();
    ...
    // My deletion method that removes keys while keeping pointers intact
    ...
    // I call this to compact my data into a permanent binary file
    void save() const {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    // My loading logic for binary snapshots
    void load() {
    ...
    // Tells me how many keys I have stored
    size_t size() const {
        std::lock_guard<std::mutex> lock(mutex_);
        size_t count = 0;
        SkipNode* curr = sentinel->tower_of_pointers[0];
    ...
    // My cleanup method for wiping the database
    void clear() {
        std::lock_guard<std::mutex> lock(mutex_);
        SkipNode* curr = sentinel->tower_of_pointers[0];
        while (curr) {
